...
### 正規表現エンジンとオートマトン — Thompson NFA・DFA変換・ReDoS の病理 (2026-05-21)
- **中心命題**: 「正規表現は線形時間で確実に照合できる」という結果は **Thompson 1968 CACM** で実用化済みだったのに、主流エンジン（Perl/PCRE/Python re/Java/JS V8）の大半は最悪指数時間のバックトラッキング型を採用——Russ Cox "Regular Expression Matching Can Be Simple And Fast"(2007) が暴いたギャップ。病的パターン `a?ⁿaⁿ` で **n=29 で Perl 60秒超 vs Thompson NFA 20μs（百万倍差）**、n=100 で Perl 理論上 10¹⁵ 年
- **形式言語の三位一体（Kleene 1956）**: 正規表現 = 有限オートマトン = 正規言語。**DFA**（遷移一意、照合 O(n) だが状態爆発しうる）vs **NFA**（複数遷移+ε遷移、サイズは正規表現長に線形）。**Rabin-Scott 部分集合構成（1959）** で NFA→DFA だが最悪 2^k 状態
- **Thompson 構成法**: 各演算子に ε遷移つき部品を再帰組立、**文字1個＝状態1個**（NFA サイズが線形＝照合線形性の前提）。1968 年は NFA 状態を機械語小片として表現する JIT 的発想
- **NFA 多状態シミュレーション = 線形時間の鍵**: 「現在到達可能な状態**集合**を1つ持ち入力1文字で一斉に進める」→ 状態数高々 m で **O(mn)**。**効率の源泉は「どの経路で来たか」を捨てること**——バックトラッキングは経路を覚えるから 2ⁿ 通り辿る。事実上 DFA の遅延構築（on-the-fly）
- **Glushkov 構成（position automaton, 1961）**: ε遷移を作らない別解、状態数＝記号数+1。Thompson NFA から ε除去すると Glushkov NFA（Mohri）
- **Unix の皮肉**: Thompson は良い算法を知っていたのに ed/初期 grep はバックトラッキング。egrep（Aho, 1979）が事前計算 DFA、Pike が効率 NFA（1985, sam）を書いたが**注目されず**、Spencer のバックトラッキング再実装が Perl/PCRE/Python の祖先に——**速い算法が忘れられ遅い算法が標準化された経路依存**
- **後方参照（backreference `\1`）という麻薬**: 「捕まえたまさにその部分文字列の繰り返し」を要求し **`{wcw}` など非正規言語を表現**（ポンプの補題の限界を踏み越える）→ **「正規表現」はもはや形式言語論の正規言語ではない**。**Aho: 後方参照付きマッチングは NP完全**、包含・等価・普遍性判定は決定不能。NFA の有限状態集合では無限多様な「捕まえた文字列」を表せない＝後方参照は多状態シミュレーションと構造的に相容れない。「機能要求が計算量クラスを決める」
- **Catastrophic backtracking / ReDoS**: `(a+)+$` 等の曖昧反復ネストで入力分割 2ⁿ 通りを網羅。攻撃者が病的入力で 1リクエスト1コア占有 → DoS
- **Cloudflare 2019-07-02 障害**: WAF の XSS ルール（PCRE）の `.*.*=.*` が `=` 無し入力で爆発。`x=x`=23ステップ、x20個=**555ステップ**、`;`付きで**5,353ステップ**。**全エッジ一斉デプロイ→CPU 100%・トラフィック80%減・27分障害**。結論「バックトラッキングエンジンから移行せよ」→ RE2/Rust regex へ
- **線形保証エンジンの設計**: **RE2**（Google, Cox, 2006）は「バックトラッキングでしか解けない構文（後方参照・look-around）を**意図的に削除**」→ 線形時間保証。最大4 DFA + 遅延構築 + LRU キャッシュ、予算超過で**指数時間にならない NFA へフォールバック**。Go regexp / Rust regex (RFC 0042) も同哲学
- **核心洞察**: 「名前が実体を超えた」希有例（正規表現⊋正規言語）。**計算量クラスを機能セットが切り替える**——後方参照 1 機能で線形↔NP困難が反転。[[計算可能性理論 — チューリングマシン・停止問題・Rice の定理]] の「決定不能を回避＝入力（言語）を制限する工学」と同型（eBPF verifier・全関数型言語と同じパターン）。「履歴を捨てて速くなる」反直観は [[Burrows-Wheeler 変換 (BWT) と FM-index]] の逆順処理・HyperLogLog の正確さ放棄と同系譜。**削除こそ最大の設計成果**（[[TLS 1.3 ハンドシェイクの設計哲学]] と同じ「制限が安全を生む」）。Thompson 1968→RE2 2006 の 40 年遅延は線形型・ZKP と同じ「基礎研究の数十年遅れの翻訳」
...
- **線形型 vs アフィン型** — 線形型は「ちょうど 1 回使用」、アフィン型は「最大 1 回使用」（捨てることを許す）。**Rust はアフィン型**、Linear Haskell は線形型
- **Rust の所有権モデル** — `let s2 = s1` で **move** され s1 は無効化。**Copy 型**（プリミティブ）はアフィン制約から免除。これは線形論理の `!P`（指数）に対応
- **借用 (Borrowing)** — `&T`（不変借用、複数同時 OK）と `&mut T`（可変借用、同時に 1 つだけ）。**aliasing-xor-mutability** ルールがデータ競合を静的に防ぐ。ライフタイム `'a` で「借用が借用元より長く生きない」保証
- **GC なしのメモリ安全** — C/C++ は手動で脆い、Java/Go は GC コスト、Rust は所有権で静的保証＋ゼロランタイムコスト。Microsoft の調査で Windows 脆弱性の 70% がメモリ問題 → Rust 移行でこのカテゴリが**設計上不可能**に。Linux カーネル 2022 公式採用、Android 推奨
- **RAII** — `Drop` トレイトで「値がスコープを抜けるとき自動デストラクタ」。リソース（ファイル・ロック・接続）が所有権スコープと結びつき、**リーク原理的に発生しない**
- **Linear Haskell** — GHC 9.0+ で `%1` multiplicity 注釈 `f :: a %1 -> b`。「既存 Haskell コードを破壊せず線形性を選択的に追加」。Rust の最初から所有権設計 vs Haskell の漸進的拡張
- **Substructural Logic** — 構造規則の選択的除去で多様な型システムが導かれる：収縮+弱化除去=線形、弱化のみ=アフィン、構造規則保持=古典
- **領域型 (Region Types)** の系譜——Tofte/Talpin 1994、Cyclone（C 拡張）→ Rust ライフタイムへ
- **限界と逃げ道** — 自己参照構造（双方向リスト、循環参照）は `Rc<T>`/`RefCell<T>`/`unsafe` で対応。`Pin<T>` は async/await の自己参照ジェネレータ向け（2018）
- **核心洞察** — 線形論理から実用言語までの **「数十年遅れの翻訳」** は基礎研究の典型。「制約が自由を生む」型システムの逆説（並行プログラムが安全に書ける）。「リソースとしての値」という認識転換 — 計算機概念モデルを物理世界に近づける。「経験的ベストプラクティスを言語仕様にコード化する」トレンド（Curry-Howard 同型の延長）
